#!/usr/bin/env python3
"""
Re-run Echo simulations for problematic points (τc = 0.300, 0.350 μs)
Post-meeting improvements: Increase N_traj and T_max_echo
"""
...
# Problematic tau_c values
problem_tau_cs = [0.300e-6, 0.350e-6]  # s
...
def main():
    print("="*80)
    print("Re-run Echo Simulations for Problematic Points")
    print("="*80)
    print(f"\nProblematic tau_c values: {[tau_c*1e6 for tau_c in problem_tau_cs]} μs")
    print(f"N_traj = {N_traj_improved} (increased from 2000)")
    print(f"\nImprovements:")
    print(f"  - T_max increased for crossover regime (25× → 30×)")
    print(f"  - T_max_echo increased (3.5× → 4.0×)")
    print(f"  - More tau_list points (60 → 80)")
    print("\nStarting simulation...\n")
    ...
    if echo_file.exists():
        ...
        df_echo = pd.read_csv("results_comparison/t2_vs_tau_c.csv")[['tau_c', 'xi']].copy()
        df_echo['T2_echo'] = np.nan
        df_echo['T2_echo_lower'] = np.nan
        df_echo['T2_echo_upper'] = np.nan
        df_echo['R2_echo'] = np.nan
    ...
    for i, tau_c in enumerate(problem_tau_cs):
        ...
        if xi > 3:  # QS regime
            T_max_echo = T_max * 3.5
        elif xi < 0.3:  # MN regime
            T_max_echo = T_max * 3.0
        else:  # Crossover - IMPROVED
            T_max_echo = T_max * 4.0  # Increased from 3.5
        ...
        tau_list = get_dimensionless_tau_range(
            tau_c, n_points=80, upsilon_min=0.05, upsilon_max=upsilon_max,  # More points
