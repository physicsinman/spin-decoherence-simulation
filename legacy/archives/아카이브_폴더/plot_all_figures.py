...
def plot_T2_vs_tau_c(data, output_dir):
        ...
        if len(qs_T2) > 5:
            ...
            if qs_T2_cv < 0.05:
                ...
                if saturation_start_idx is not None:
                    ...
                    # IMPROVEMENT: Move annotation to right edge, positioned between parameter and crossover note
                    ax.text(0.995, 0.50, 'Saturation\n(limitation)', 
    ...
    # IMPROVEMENT: Legend moved to upper right, smaller size, positioned to avoid overlap
    ax.legend(loc='upper right', frameon=True, fancybox=True, shadow=True, fontsize=8, 
             bbox_to_anchor=(0.995, 0.98))
    ax.grid(True, alpha=0.3, linestyle='--', linewidth=0.8)
    
    # IMPROVEMENT: Parameter info moved to right edge, positioned below legend
    ax.text(0.995, 0.75, param_text, transform=ax.transAxes, fontsize=7,
           verticalalignment='top', horizontalalignment='right',
           bbox=dict(boxstyle='round,pad=0.4', facecolor='white', alpha=0.8, edgecolor='gray'),
           family='monospace')
    
    # IMPROVEMENT: Add note about crossover regime (no analytical solution)
    # Place in lower right corner (away from data)
    crossover_note = 'Note: Crossover regime (0.5 ≤ ξ < 2.0)\nhas no analytical solution'
    ax.text(0.995, 0.02, crossover_note, transform=ax.transAxes, fontsize=8,
           verticalalignment='bottom', horizontalalignment='right',
...
def plot_representative_curves(data, output_dir):
    ...
    for idx, tau_c_key in enumerate(selected_keys):
        ...
        if 'time (s)' in fid_df.columns and E_col is not None:
            ...
            # IMPROVEMENT: Increased linewidth for publication quality (1.8-2.0)
            ax.plot(t * 1e6, E, '-', color=COLORS['fid'], linewidth=2.0,
                   label='FID', alpha=0.8)
        
        # NOTE: Echo curves are NOT plotted here because echo_tau_c_*.csv files
        # contain τ (pulse delay) sweep data, NOT time evolution data.
        # Plotting them as time evolution would be physically incorrect.
        # Echo effectiveness is demonstrated in Figure 3 (echo gain).
        
        # IMPROVEMENT: Unified x-axis range (0-3 μs) for all panels
        # This allows direct comparison across regimes
        ax.set_xlim([0, 3.0])
        ...
        # IMPROVEMENT: Compact legend
        ax.legend(loc='upper right', fontsize=10, frameon=True, fancybox=True, shadow=True)
        # IMPROVEMENT: Softer grid (alpha=0.25, linewidth=0.5)
        ax.grid(True, alpha=0.25, linestyle='--', linewidth=0.5)
        # IMPROVEMENT: ymin = -0.02 to show noise floor naturally
